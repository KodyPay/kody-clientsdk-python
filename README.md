...
- `terminal_id` – the serial number of the terminal that will process the payment request (returned by the list of terminals or found on the device)
- `amount` – amount as a 2.dp decimal number (e.g. `"1.00"`)
- `show_tips` – (optional) whether to display tip options (default is false)
- `payment_method` – (optional) settings to enable direct QR scanning
  - `payment_method_type` – Payment method type: CARD (default) or E_WALLET
...
class PayResponse:
    ...
    failure_reason: Optional[str] = None  # Populated on failure
    receipt_json: Optional[str] = None    # JSON blob for receipt data
...
payment_request = kody_model.PayRequest(
    store_id="STORE ID",
